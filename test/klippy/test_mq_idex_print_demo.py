...
# Marathon evidence (mq.cfg / PARK_extruder / PARK_extruder1):
#   PARK_extruder  -> G1 X0
...
class TestIdexPrintDemo(unittest.TestCase):
    ...
    def test_print_demo_multi_toolchange_shared_y(self):
        # Longer Formbot-shaped CUT (not production print):
        #   T0 print + claim Y shared move + release
        #   -> TOOLCHANGE T1 (park X0) -> T1 print
        #   -> TOOLCHANGE T0 (park X433) -> T0 print + claim Z
        ...
        # --- TOOLCHANGE T1->T0: park X433 ---
        self.toolchange(obj, 'T0')
        self.assertEqual(obj.current.name, 'T0')
        self.assert_park_script(
            gcode.scripts[-1], 433, 'q_T1', 'q_T0')
        ...
        # Three TOOLCHANGE emits with Formbot parks only
        ...
        for s in tc_scripts:
            if 'G1 X0' in s:
                ...
                parks.append(433)
        self.assertEqual(parks, [0, 433, 0])
        for s in tc_scripts:
            self.assertNotIn('X434', s)

    def test_print_demo_ownership_and_stock_tc(self):
        # Overlay-shaped ownership + Formbot parks; stock no-queue
