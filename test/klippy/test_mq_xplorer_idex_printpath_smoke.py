# Short IDEX print-path smoke (Xplorer-shaped MQ).
# TOOLCHANGE + SET_MOTION_QUEUE + dual_carriage motion on both
# queues. Parks from [toolchange] only (Xplorer X0 / X421).
...
# Do NOT use Marathon X433 or stock dual_carriage X200.
# Evidence: configs/mq/xplorer/mq-idex.cfg (Plus #2 -d PASS).
TWO_QUEUE_TC_CFG = """
[printer]
...
class TestXplorerIdexPrintpathSmoke(unittest.TestCase):
    ...
    def test_printpath_toolchange_and_both_carriages(self):
        ...
        # TOOLCHANGE T0 (park X421) -> T0 print segment.
        ...
        # --- toolchange back to T0: outgoing park X421 ---
        self.toolchange(obj, 'T0')
        self.assertEqual(obj.current.name, 'T0')
        script_t0 = gcode.scripts[-1]
        self.assertIn('G1 X421', script_t0)
    ...
    def test_printpath_ownership_parks_xplorer_only(self):
        # Config parks are Xplorer X0 / X421 only; ownership maps
